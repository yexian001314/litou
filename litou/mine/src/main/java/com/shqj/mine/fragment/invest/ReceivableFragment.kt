...
import kotlinx.android.synthetic.main.mine_fragment_receivable.*


/**
 * 我的投资 -> 已还款
 */
class ReceivableFragment : UUBaseListFragment<MultiItemEntity, InvestRecordBean>() {
    ...
    override fun getLayoutResourse(): Int {
        return R.layout.mine_fragment_receivable
    }

    override fun initItemLayout() {
        super.mRecyclerView = receivable_recycler
        super.mSmartLayout = receivable_refresh
        ...
        mAdapter?.onItemClickListener = BaseQuickAdapter.OnItemClickListener { adapter, view, position ->
            if (!LibCommonUtil.isFastDoubleClick()) {
                if (mAdapter?.data!![position] is InvestRecordBean.PageBean.DataBean) {
                    MobclickAgent.onEvent(context, "wd_wdtz_yhk_lbdj")
                    ARouter.getInstance()
    ...
    override fun customAdapter(): Boolean {
        super.mAdapter = MyinvestExpendAdapter(this.context, null, object : MyinvestExpendAdapter.ExpandListener {
            override fun expanded(position: Int) {
                MobclickAgent.onEvent(context, "wd_wdtz_yhk_jtdj")
...
//                        mAdapter?.setNewData(newData)
    ...
    private fun getData() {
        mPresenter.getMyInvestData(REQUEST_PAGE, 3)
    }
}
