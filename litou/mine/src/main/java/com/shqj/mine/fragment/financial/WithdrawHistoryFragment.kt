...
import kotlinx.android.synthetic.main.mine_fragment_withdraw_history.*
 ...
 * 资金明细 -> 提现记录
 */
class WithdrawHistoryFragment : UUBaseListFragment<InvestDetailBean.ProjectsBean.DataBean, InvestDetailBean>() {

    private var REQUEST_PAGE = 0
    private lateinit var mPresenter: FinancialDetailPresenter

    override fun getLayoutResourse(): Int {
        return R.layout.mine_fragment_withdraw_history
    }

    override fun initItemLayout() {
        super.mRecyclerView = withdraw_history_recycler
        super.mSmartLayout = withdraw_history_refresh
    ...
    override fun commonHolder(viewHolder: BaseViewHolder, data: InvestDetailBean.ProjectsBean.DataBean) {
        //类型
        viewHolder.setText(R.id.tv_type, getString(R.string.withdraw))
        //创建时间
    ...
    private fun getData(){
        mPresenter.getFinancialDetailData(REQUEST_PAGE,2)
    }
}
