...
import kotlinx.android.synthetic.main.mine_fragment_invest_history.*
 ...
 * 资金明细 -> 投资记录
 */
class InvestHistoryFragment : UUBaseListFragment<InvestDetailBean.ProjectsBean.DataBean, InvestDetailBean>() {

    private var REQUEST_PAGE = 0
    private lateinit var mPresenter: FinancialDetailPresenter

    override fun getLayoutResourse(): Int {
        return R.layout.mine_fragment_invest_history
    }

    override fun initItemLayout() {
        super.mRecyclerView = invest_history_recycler
        super.mSmartLayout = invest_history_refresh
    ...
    override fun commonHolder(viewHolder: BaseViewHolder, data: InvestDetailBean.ProjectsBean.DataBean) {
        //类型
        viewHolder.setText(R.id.tv_type, getString(R.string.invest))
        //创建时间
    ...
    private fun getData(){
        mPresenter.getFinancialDetailData(REQUEST_PAGE,3)
    }
