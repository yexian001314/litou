...
public class CountedEditText extends android.support.v7.widget.AppCompatEditText{
    ...
    private void initAttrs(AttributeSet attrs) {
        ...
        try {
            ...
            mCountedSize = AutoUtils.getPercentHeight1px()*mTypedArray.getDimension(R.styleable.CountedEditText_countSize, 40);
            mOverCountColor = mTypedArray.getColor(R.styleable.CountedEditText_overCountColor, Color.RED);
            mCountMaxNum = mTypedArray.getInteger(R.styleable.CountedEditText_maxNum, 40);
...
//        // 获取额外的顶部内边距
//        extraTopPadding = (int) getTextHeight(countPaint) + dpToPix(4);
//
//        // 获取额外的底部内布距
//        extraBottomPadding = (int) getTextHeight(countPaint) + dpToPix(6);
        // 初始化监听器
        initListener();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float mTextHeight = getTextHeight(countPaint);
        if (mCountMaxNum != -1) {
